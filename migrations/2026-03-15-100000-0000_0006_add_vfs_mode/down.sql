...
-- Recreate the table without the mode column.
CREATE TABLE vfs_files_backup AS SELECT
    id, drive, connection_id, session_id, user_id, path,
    is_directory, size_bytes, content_type, inline_data,
    media_hash, modified_by, created_at, updated_at
FROM vfs_files;
...
INSERT INTO vfs_files SELECT * FROM vfs_files_backup;
